...
{	
	...
	if(!isset($_FILES['mFile']))
	{
		...
		$query = mysql_query("UPDATE projects SET title = '$FileTitle',discr = '{$_POST['descr']}' WHERE id= '{$_POST['id']}'")or die(mysql_error());
                die("success");
	}else{
...
//        $layer->applyFilter(IMG_FILTER_BRIGHTNESS, 9, null, null, null, true);
        ...
        $query = mysql_query("UPDATE projects SET title = '$FileTitle',discr = '{$_POST['descr']}',image = '{$filename}' WHERE id= '{$_POST['id']}'")or die(mysql_error());
        die("success");
   }else{
